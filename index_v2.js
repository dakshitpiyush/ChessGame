...
function autocalculate(user){
    var possmov={}; //this variable is diffrant than global one
    for(var i=0;i<8;i++){
        for(var j=0;j<8;j++){
            if(board[i][j].length>0){
                if(board[i][j][0].className == user){
                    possmov[i+""+j]=[];
                    switch(board[i][j][0].name){
                        case "slder":
                            if(user=="white"){
                                if(i==6){
                                    var inc=1;
                        ...
                        case "horse":
                            ...
                            if(x<8 && y<8 && (board[x][y].length==0 || board[x][y][0].className!=user)){
                                possmov[i+""+j].push(x+""+y);                            }
                            y=j-1;
                            if(x<8 && y>=0 && (board[x][y].length==0 || board[x][y][0].className!=user)){
                                possmov[i+""+j].push(x+""+y);                            }
                            x=i-2;
                            if(x>=0 && y>=0 && (board[x][y].length==0 || board[x][y][0].className!=user)){
                                possmov[i+""+j].push(x+""+y);                            }
                            y=j+1
                            if(x>=0 && y<8 && (board[x][y].length==0 || board[x][y][0].className!=user)){
                                possmov[i+""+j].push(x+""+y);                            }
                            x=i+1; y=j+2;
                            if(x<8 && y<8 && (board[x][y].length==0 || board[x][y][0].className!=user)){
                                possmov[i+""+j].push(x+""+y);                            }
                            ...
                            case "king":
                                var x=i+1,inc=0;
                                if(x<8 && (board[x][j].length==0 || board[x][j][0].className!=user)){
                                    possmov[i+""+j].push(x+""+j);                                }
                                x=i-1;
                                if(x>=0 && (board[x][j].length==0 || board[x][j][0].className!=user)){
                                    possmov[i+""+j].push(x+""+j);                                }
                                x=j+1;
                                if(x<8 && (board[i][x].length==0 || board[i][x][0].className!=user)){
                                    possmov[i+""+j].push(i+""+x);                                }
                                x=j-1;
                                if(x>=0 && (board[i][x].length==0 || board[i][x][0].className!=user)){
                                    possmov[i+""+j].push(i+""+x);                                }
                                x=i-1;
                                var y=j-1;
                                if(x>=0 && y>=0 && (board[x][y].length==0 || board[x][y][0].className!=user)){
...
update();
var unfi = [];
unfi["white"] = autocalculate("white");
unfi["black"] = autocalculate("black");
possmov = getfilter(unfi["white"]);
...
async function markPossible(source){
    ...
    if(source.children.length>0 && source.lastChild.classList[0]=="dot"){
        ...
        track["self"]["source"] = track["oponant"]["source"];
        track["self"]["dest"] = track["oponant"]["dest"];
        track["oponant"]["source"] = cur_pos;
        track["oponant"]["dest"] = source.id;
        cur_pos='';
        currenttroop='';
        checkbit=ifCheck();
        update();
        swapuser();
        possmov=getfilter(autocalculate(user));
        var res = result();
...
function update_pos() {
    if (track.self.source != "") {
        let i = track.self.source[0];
        let j = track.self.source[1];
        for (arre in unfi[user]) {
            if (arre == track["oponant"]["dest"]) {
                delete unifi[user][arre];
            }
            let val = (i + 1) + "" + j;
            if (arre == val || unifi[user][arre].includes(val)) {
                switch (board[arre[0]][arre[1]][0].name) {
                    case "slder":
                        if (user == "black" && unfi["black"][arre].length < 2) {
                            if (arre[0] == 1) {
                                var k = 1;
                                var inc = 1;
                                while (inc <= 2) {
                                    var x = 1 + inc;
                                    if (board[x][arre[1]].length > 0) {
                                        break;
                                    }
                                    unfi["black"][inc - 1] = x + ""+arre[1];
                                }
                            } else {
                                if (unfi["black"][arre].length < 1) {
                                    unfi["black"][]
                                }
                            }
                        }
            ...
            val = (i - 1) + "" + j;
            if (arre == val || unifi[user][arre].includes(val)) {
                //queen,king,elephant& slder if white    
            }
            val = i + "" + (j - 1);
            if (arre == val || unifi[user][arre].includes(val)) {
                //queen,king,elephant   
            ...
            var x = i + 2, y= j + 1;
            if (arre == (x+""+y) && board[x][y][0].name=="horse") {
                unfi[user][arre].push(i + "" + j);
            }
            x = i + 2, y = j - 1;
            if (arre == (x + "" + y) && board[x][y][0].name=="horse") {
                unfi[user][arre].push(i + "" + j);
